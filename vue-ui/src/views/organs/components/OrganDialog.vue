<template>
  ...
  >
    <el-form
      ...
      :disabled="formMode"
    >
      <el-form-item label="组织/器官名：" prop="organName">
        <el-col :span="halfSpan">
          <el-input v-if="!formMode" v-model="form.organName" />
          <div v-else v-html="form.organName"></div>
        </el-col>
      </el-form-item>
...
import { addOrganRecord, editOrganRecord } from '@/api/organ'
...
const emits = defineEmits(['update:modelValue', 'initOrganRecords'])
    ...
    formRef.value.validate(async (valid) => {
      if (valid) {
        if (props.dialogTitle.includes('添加')) {
          await addOrganRecord(form.value)
        } else {
          await editOrganRecord(form.value)
        }
