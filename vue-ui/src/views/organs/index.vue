  ...
  <div class="organ-view">
    <el-card>
      <el-row :gutter="20" class="header">
        ...
        <el-button type="primary" :icon="Search" @click="initGetOrganRecords"
          >搜索</el-button
        >
    ...
    <div>
      <organ-dialog
        ...
        @initOrganRecords="initGetOrganRecords"
      />
    </div>
  </div>
</template>
...
<script setup>
import OrganDialog from './components/OrganDialog.vue'
import { ref } from 'vue'
import { Search, Plus, Picture } from '@element-plus/icons-vue'
import { isNull } from '@/utils/filter'
import { options } from './options'
import { ElMessageBox, ElMessage } from 'element-plus'
import { getOrganRecords, deleteOrganRecord } from '@/api/organ'
...
const handleDialogValue = (row) => {
  if (isNull(row)) {
    dialogTitle.value = '添加数据：人体组织器官信息'
    dialogTableValue.value = {}
  } else {
    dialogTitle.value = '编辑数据：人体组织器官信息'
    console.log(row)
...
const initGetOrganRecords = async () => {
  const res = await getOrganRecords(queryForm.value)
...
const deleteRecord = (row) => {
  ...
  ElMessageBox.confirm('确定要删除记录', 'Warning', {
    ...
    .then(async () => {
      await deleteOrganRecord(row.organId)
      ElMessage({
        type: 'success',
        message: '删除成功'
      })
...
const displayData = (row) => {
  dialogTitle.value = '预览数据：人体组织器官信息'
  dialogTableValue.value = JSON.parse(JSON.stringify(row))
  dialogVisible.value = true
}
